...
{
    public class NodeDefineSpecialSTGObject : BaseNode, IEventReciver
    {
        public override void Init(RectTransform parentTf)
        {
            _nodeType = NodeType.DefineSpecialSTGObject;
            _extraDepth = 0;
            base.Init(parentTf);
        ...
        {
            // 添加OnCreate子节点
            BaseNode onCreateNode = NodeManager.CreateNode(NodeType.OnSpecialSTGObjectCreate);
        ...
        public override void OnAttributeValueChanged(BaseNodeAttr attr = null)
        {
            if (attr != null)
            {
                if (attr.GetPreValue() == null)
                {
                    ...
                    if (newTypeName != "")
                    {
                        BaseNode onCreateNode = GetChildByType(NodeType.OnSpecialSTGObjectCreate);
            ...
            else // 载入节点数据or设置节点默认值时
            {
                ...
                if (typeName != "")
                {
                    BaseNode onCreteNode = GetChildByType(NodeType.OnSpecialSTGObjectCreate);
