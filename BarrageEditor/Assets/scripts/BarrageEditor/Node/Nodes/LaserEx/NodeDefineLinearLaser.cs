...
namespace BarrageEditor
{
    public class NodeDefineLinearLaser : BaseNode, IEventReciver
    {
            ...
            if (attr != null)
            {
                if (attr.GetPreValue() == null)
                {
                    ...
                    if (newTypeName != "")
                    {
                        ...
                        _isWatchingData = CustomDefine.AddData(CustomDefineType.LinearLaser, newTypeName, onCreteNode.GetAttrByIndex(0).GetValueString());
                    }
                }
                else
                {
                    ...
                    if (fromName != "")
                    {
                        ...
                        {
                            BaseNode onCreteNode = GetChildByType(NodeType.OnLinearLaserCreate);
                            _isWatchingData = CustomDefine.AddData(CustomDefineType.LinearLaser, attr.GetValueString(), onCreteNode.GetAttrByIndex(0).GetValueString());
                        }
                    }
                    else
                    {
        ...
        {
            return string.Format("define linear laser of type \"{0}\"", _attrs[0].GetValueString());
        }
