...
namespace BarrageEditor
{
    public class NodeDefineLaser : BaseNode, IEventReciver
    {
            ...
            if (attr != null)
            {
                if (attr.GetPreValue() == null)
                {
                    ...
                    if (newTypeName != "")
                    {
                        ...
                        _isWatchingData = CustomDefine.AddData(CustomDefineType.Laser, newTypeName, onCreteNode.attrs[0].GetValueString());
                    }
                }
                else
                {
                    ...
                    if (fromName != "")
                    {
                        if (_isWatchingData)
                        {
                            CustomDefine.ModifyDefineName(CustomDefineType.Laser, fromName, attr.GetValueString());
                        }
                        else
                        {
                            BaseNode onCreteNode = GetChildByType(NodeType.OnLaserCreate);
                            _isWatchingData = CustomDefine.AddData(CustomDefineType.Laser, attr.GetValueString(), onCreteNode.attrs[0].GetValueString());
                        }
                    }
                    else
                    {
        ...
        {
            return string.Format("define laser type \"{0}\"", attrs[0].GetValueString());
        }
